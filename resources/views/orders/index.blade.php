                                           ...
                                           @foreach($order->items as $index => $item)
                                               <tr>
                                                   <td class="product-info">
                                                       <div class="preview">
                                                           <a href="{{ route('products.show', ['product' => $item->product->id]) }}" target="_blank">
                                                               <img src="{{ $item->product->image_url }}">
                                                           </a>
                                                       </div>
                                                       <div>
                                                           <span class="product-title">
                                                               <a href="{{ route('products.show', ['product' => $item->product->id]) }}" target="_blank">{{ $item->product->title }}</a>
                                                           </span>
                                                           <span class="sku-title">{{ $item->productSku->title }}</span>
                                                       </div>
                                                   </td>
                                                   <td class="text-center sku-price">￥{{ $item->price }}</td>
                                                   ...
                                                   @if($index == 0)
                                                       ...
                                                       <td rowspan="{{ count($order->items) }}" class="text-center">
                                                           ...
                                                           @else
                                                               未支付<br>
                                                               请于{{ $order->created_at->addSeconds(config('app.order_ttl'))->format('H:i') }}前支付<br>
                                                               否则订单自动关闭
                                                           @endif
                                                       </td>
                                                       <td rowspan="{{ count($order->items) }}" class="text-center">
                                                       ...
                                                       {{-- 评论入口start --}}
                                                       @if($order->paid_at)
                                                           <a href="{{ route('orders.review.store', [$order->id]) }}" class="btn btn-success btn-sm">{{ $order->reviewed ? '查看评论' : '评价' }}</a>
                                                       @endif
                                                       {{-- 评论入口end --}}
                                                   @endif
                                               </tr>
